...
class MultiModalClusteringMetric():
    ...
    def __init__(self, 
        ...
        self.llava_results_df["category_llava"] = self.llava_results_df["category_llava"].apply(
                    lambda cat: cat if cat in self.categories or cat == "NOT VALID" or cat == "NOT RELEVANT" else "BAD_INFERENCE"
                    )
        self.cache = cache
        ...
        self.category_colors["NOT RELEVANT"] = "red"
        self.category_colors["BAD_INFERENCE"] = "black"
        self.unknown_category_color = "black"  # Reserved for unknowns


        # Base dirs
        self.results_dir = Path(__file__).resolve().parent / f"results/{self.experiment_name}/ground_truth"
    ...
    def generate_stats_multiple_categories(n_cat=2):
        """ 
    ...
    def plot_cluster_categories_2(self, threshold=0.75):
        ...
        model_clustering = self.clustering
        eval_method = self.eval_method
        score_best = self.score
        ...
        if not noise_data.empty:
            ...
            ax.set_title(
                ...
                f"Experiment ID: {1}, Classification Level: {self.classification_lvl}, "
                f"Prompt: {self.n_prompt}, Llava Model: {self.model}\n"
                f"Evaluation Method: {self.eval_method}, Score (w/o Penalty): {self.score:.4f}, "
    ...
    def plot_cluster_categories_3(self, threshold=0.75):
        ...
        fig, axes = plt.subplots(2, 1, figsize=(55, 70))
        # fig.suptitle(
        #     f"Category Distribution by Cluster (Excluding Noise)\n"
