...
class Dinov2Inference:
    ...
    def run(self):
        ...
        if not self.cache or not self.embeddings_cache_path.exists() or not self.__is_cache_the_same():
        ...
        # If we want cache
            ...
            logger.info(
                f"Same number of images from Data. Disave cache by calling --cache=False {self.embeddings_cache_path}"
            )
            logger.info(
